...
export default function NumberGamesPage() {
  return (
    <main id="main" className="mx-auto flex w-full max-w-5xl flex-1 flex-col justify-center px-5 pb-16 pt-6 sm:px-8 sm:pt-10">
      ...
      <nav aria-label="数字小游戏" className="grid gap-5 sm:grid-cols-2 sm:gap-7 md:grid-cols-2 md:gap-4 lg:grid-cols-4">
        {numberGames.map((game, index) => (
          <Link
            key={game.path}
            to={game.path}
            className={`group relative block min-h-64 overflow-hidden rounded-[2rem] border-2 bg-gradient-to-br p-7 text-left shadow-lg transition duration-300 hover:-translate-y-2 hover:shadow-2xl active:scale-[0.98] sm:min-h-80 sm:p-9 md:min-h-72 md:p-6 lg:p-5 ${game.card}`}
          >
